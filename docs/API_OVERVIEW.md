...
- [ ] 擴充第 3 節，依模組（auth / user / system / ai-chat / billing / secret / ai-provider /
      ai-model / ai-agent 等）逐步補齊代表路由與職責摘要。
- [ ] 明確整理 console API 前綴（`/consoleapi/**`）與各子系統（Provider / Model / Secret / Agent /
      Billing...）的對應關係。
