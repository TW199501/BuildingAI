...
> 狀態：已完成第一輪 `console/ai/provider` 相關頁面閱讀，以下內容僅針對「AI Provider 管理」這個子系統的前端 UI flow 與資料流做整理；後端 provider 模組與 secret 模組的實作細節暫不展開。
...
> 目標：描述使用者進入 console 後，如何進到 AI Provider 管理頁、查看列表、進行新增／編輯／刪除與啟用／停用等操作的畫面流程。
    ...
    - 元件：
      ...
      - `USelect` 綁 `searchIsActive`（實際寫入 `searchForm.isActive`）：
        - items：
          ...
          - show（顯示中）。
          - hide（隱藏／停用）。
        - `@change="handleIsActiveChange"`：內部會轉換成 `searchForm.isActive = undefined | true | false`，再呼叫 `getLists()` 刷新。
      - 右側操作區（`md:ml-auto` 對齊右邊）：
        ...
        - 批次刪除按鈕（包在 `AccessControl` 裡）：
          - `UButton`（color=error, variant=subtle），點擊呼叫 `handleBatchDelete`，會彈出確認 modal，成功後重新載入列表。
    ...
    - 當 `!loading && providers.length > 0`：
      - `BdScrollArea`（高度 `h-[calc(100vh-12rem)]`）包一個 `grid`：
        - `ProviderCard` v-for 渲染每一個 Provider：
          ...
          - emits：`@select` / `@delete` / `@edit` / `@view-models` / `@toggle-active` 對應列表頁的方法。
    - 當 `loading`：顯示 loading spinner 與載入中文文案。
    - 當 `providers.length === 0` 且非 loading：
      - 顯示空狀態圖示、文案與一個「新增第一個 Provider」的 UButton（呼叫 `handleAddProvider`）。
  - 底部 **統計列（sticky 在底部）**：
...
- **新增／編輯 Provider 的畫面流**
  - 從列表頁點擊：
    ...
    - 編輯：
      - 在 ProviderCard 中點擊「編輯」 → 呼叫 `handleEditProvider(provider)` → `mountProviderEditModal(provider.id)`。
  - `mountProviderEditModal` 內部：
    ...
    - 等待 modal 關閉：`const shouldRefresh = await instance.result`，若 `true` 則重新呼叫 `getLists()` 重載列表。
  - 使用者在 `edit.vue` 中填寫或修改表單後：
    - 點擊「建立」或「更新」按鈕送出，成功會 `emits("close", true)`，讓列表頁知道需要 `getLists()` 刷新。
...
- **新增／編輯 modal 的開啟與關閉**
  ...
  - 當 `edit.vue` 中成功送出時會 emit `close(true)`，使得列表頁收到 `shouldRefresh = true`，接著 `getLists()`。
...
- **支援模型類型資料**
  - `allModelTypes: shallowRef<ModelType[]>`：
    - 透過 `useAsyncData("model-types", apiGetAiProviderModelTypes)` 取得（在 script setup 頂層 await）。
...
> 目標：說明 Provider 管理 flow 如何與權限控制、Secret 管理、模型／Agent 管理與 chat C-API 組合成一條整體鏈路。
...
- **與模型與 Agent / Chat 流程的關係**
  - Provider 上設定的：
    - `supportedModelTypes`、`bindSecretId`、`isActive` 等資訊，會影響：
      ...
      - chat C-API 在 `ModelValidationCommandHandler` 取 model + provider + secret 時是否能成功。
...
- [ ] 若未來要深入 Provider module 的後端 API flow（Provider 與 Secret / AiModel 的關係），可另開 D-API 類型 TODO（類似 C-API 深入）。
- [ ] 視需求補充：ProviderCard / ModelModal / KeyPoolSelect 的細節，以支援更細緻的 console UX 設計或重構。
